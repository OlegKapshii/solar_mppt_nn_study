function solar_sim3()
    ...
    function update_sim(~, ~)
        ...
        V_mpp_nominal = 30;
        V_mpp_ref = V_mpp_nominal;
        T_ref = 25;
        NOCT = 45;
        beta_vmp = -0.0035; % Температурний коефіцієнт V_mpp (1/°C)
        G_ref = 1000;
        ...
        % Додаємо просту температурну модель, рухомий V_mpp та втрати перетворювача.
        P_ideal = G_POA .* A_total * 0.18; % Доступний максимум на MPP (без втрат перетворювача)
        Tamb = 18 + 10*sin(2*pi*(t_hours - 7)/24); % Профіль температури повітря протягом доби
        Tcell = Tamb + (NOCT - 20)/800 .* G_POA;

        P_track = zeros(size(t_hours)); % Потужність, яку зняв MPPT з панелей
        P_out = zeros(size(t_hours));   % Потужність після DC-DC перетворювача

        V_curr = 15; P_prev = 0; direction = 1; % Початкові умови алгоритму зранку
        P_rated = A_total * G_ref * 0.18;

        for i = 1:length(t_hours)
            if G_POA(i) < 5, continue; end % Якщо дуже темно - мікроконтролер спить

            % V_mpp зміщується з температурою та слабко залежить від освітленості
            v_irr_corr = 0.02 * log(max(G_POA(i), 50) / G_ref);
            V_mpp_dyn = V_mpp_ref * (1 + beta_vmp*(Tcell(i) - T_ref) + v_irr_corr);
            V_mpp_dyn = max(15, V_mpp_dyn); % Захист від нереалістично низьких значень

            % Фізична модель панелі: втрати при відхиленні від поточного V_mpp
            mismatch = 1 - ((V_curr - V_mpp_dyn)/V_mpp_dyn)^2;
            P_act = P_ideal(i) * max(0, mismatch);

            % Головна логіка P&O: при падінні потужності змінюємо напрямок збурення
            if (P_act - P_prev) <= 0
                direction = -direction;
            end

            % Один крок керування на один відлік часу (реалістичніша динаміка)
            V_curr = V_curr + direction*V_step;
            V_curr = min(max(V_curr, 0.5*V_mpp_ref), 1.5*V_mpp_ref);
            P_prev = P_act;

            % ККД перетворювача: нижчий при малому навантаженні, вищий біля номіналу
            load_ratio = min(1, P_act / (P_rated + eps));
            eta_conv = 0.94 + 0.04 * sqrt(load_ratio);

            P_track(i) = P_act;
            P_out(i) = P_act * eta_conv;
        end
        ...
        plot(t_hours, P_track/1000, 'b', 'LineWidth', 1.2);
        plot(t_hours, P_out/1000, 'r', 'LineWidth', 1.5); hold off;
        grid on; xlim([4 21]);
        ...
        legend(ax2, {'Теоретичний максимум (MPP)', 'P&O на панелі', 'Після перетворювача'}, 'Location', 'northwest');
        ...
        E_track = sum(P_track)*(dt_min/60)/1000;
        E_out = sum(P_out)*(dt_min/60)/1000;

        eff_track = (E_track / E_ideal) * 100; % Ефективність відстеження MPP
        eff_system = (E_out / E_ideal) * 100;  % Загальна ефективність із втратами перетворювача

        clc;
        fprintf('--- СИМУЛЯЦІЯ ЗАВЕРШЕНА ---\n');
        fprintf('Хмарність: %g%%\n', cloud_pct);
        fprintf('Теоретично можливо: %.2f кВт*год\n', E_ideal);
        fprintf('Зібрано MPPT (на панелі): %.2f кВт*год\n', E_track);
        fprintf('Після перетворювача: %.2f кВт*год\n', E_out);
        fprintf('Ефективність трекінгу P&O: %.1f%%\n', eff_track);
        fprintf('Загальна ефективність системи: %.1f%%\n', eff_system);
    end
end
